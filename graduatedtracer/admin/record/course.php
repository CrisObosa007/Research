...
if (isset($_SESSION['id']) && isset($_SESSION['username'])) {
  //USERNAME LOGIN
    ...
    if (isset($_POST['submitcourse'])) {
      $_SESSION['recordcourse'] = $_POST['recordcourse'];
    }
    if (isset($_SESSION['recordcourse'])) {
    $college = $_SESSION['recordcourse'];
...
unset($_SESSION['adminrecordsuccess']);
 ...
 <?php if(isset($_SESSION['adminrecorddelete'])) {?>
   ...
   <?php }
   unset($_SESSION['adminrecorddelete']);
   ?>
...
<table id="myTable" class="table table-striped" width="100%" style=""> 
        ...
        <tbody>  
        <?php 
        $sql = "select * from signup where college = '$college' and status = 'Approval'";
        $result = $conn-> query($sql);
        if ($result-> num_rows > 0 ){
        while ($row=mysqli_fetch_array($result)){
                              ...
                              <td colspan='2'>
                                <div class='col-xl-12  col-md-12 mt-2'  align='center' style='box-shadow: rgba(0, 0, 0, 0.15) 0px 2px 8px;margin-bottom:20px'>
                                 <div class='p-3' style='background-color:white;width:100%;' >
                                      ...
                                      <form action='courseserver.php' method='POST' id='deletepopup' name='deletepopup' onsubmit='return validateReason()'>
