...
if(isset($_POST['removeinfo'])){
  ...
  $newsimage = $row['newsimage'];
   ...
   $newsdate= date("Y-m-d");
      ...
      $result = mysqli_query($conn,"INSERT INTO alnews (newscollege,newsdetail,description,newsimage,newsdate,newstime,newsstatus,reason) values ('admin','$newsdetail','$description','$newsimage','$newsdate','$time','removed','$reasons1')");

        $result1 = mysqli_query($conn,"DELETE FROM news WHERE newsid='$newsid'");

  $_SESSION['canceledadminnewssucess'] = "asdasdsa";
