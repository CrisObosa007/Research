...
<script type="text/javascript">
 ...
 <!-- ======= Header ======= -->
     <?php if(isset($_SESSION['adminjobhiringdelete'])) {?>
    <div id="undo-hidden">
          <input type="checkbox" id="tableundo"><label for="tableundo"  class=""></label>
                     <div class="container-undo1">
                          <div class="container-undo01" align="center">
                                ...
                                <br>
                                <label style="font-size: 2em;margin-top: 20px;">JOB CLOSED!</label>
                                </label><br>
                          </div>
                    </div>
...
unset($_SESSION['adminjobhiringdelete']);
 ?>
<body>
<div id="loading">
...
<table id="myTable1" class="table table-striped" width="100%" style=""> 
        ...
        <tbody>  
        <?php 
        $sql = "SELECT * from jobpost where jobstatus = 'approved' and collegeuploaded = 'admin' ";
        $result = $conn-> query($sql);
       if ($result-> num_rows > 0 ){
        while ($row=mysqli_fetch_array($result)){
        $startdate = $row['startdate'];
