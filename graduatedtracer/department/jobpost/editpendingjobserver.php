...
include '../dbhelper.php';
 ...
 $link = addslashes($link);
  ...
  $result = mysqli_query($conn,"UPDATE jobpost set jobtitle='$jobtitle', companyname='$companyname', email='$email', contact='$contact', link='$link', jobtype='$jobtype', location='$location', specialization='$specialization1', startdate='$startdate', enddate='$enddate', qualification='$qualification1', description='$description', minimumsalary='$minimumsalary', maximumsalary='$maximumsalary'  where id='$jobid'");
    ...
    $result1 = mysqli_query($conn,"INSERT INTO aljobpost (jobtitle,companyname,email,contact,link,startdate,enddate,jobtype,location,specialization,qualification,description,jobstatus,collegeuploaded,jobpostdate,jobposttime,minimumsalary,maximumsalary) values ('$jobtitle','$companyname','$email','$contact','$link','$startdate','$enddate','$jobtype','$location','$specialization1','$qualification1','$description','update','$college','$jobpostdate','$time','$minimumsalary','$maximumsalary')");


  if ($result) {
    $_SESSION['editdepartmentjobssucess'] = "asdasdsa";
