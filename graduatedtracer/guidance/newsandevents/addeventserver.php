...
if(isset($_POST['submit'])){
  ...
  $result = mysqli_query($conn,"INSERT INTO event (eventcollege,uploadedevent,eventdetail,startdate,starttime,enddate,endtime,venue,address,description,organizer,sponsor,eventimage,eventdate) values ('$eventcourse1','guidance','$eventdetail','$startdate','$starttime','$enddate','$endtime','$venue','$address','$description','$organizer1','$sponsor1','$allfiles','$eventdate')");


  $result = mysqli_query($conn,"INSERT INTO alevent (eventcollege,uploadedevent,eventdetail,startdate,starttime,enddate,endtime,venue,address,description,organizer,sponsor,eventimage,eventdate,time,status) values ('$eventcourse1','guidance','$eventdetail','$startdate','$starttime','$enddate','$endtime','$venue','$address','$description','$organizer1','$sponsor1','$allfiles','$eventdate','$time','uploaded')");




  require "../mail.php";
  $spesplit = explode(',,,', $eventcourse1);  
  for($i=0; $i<sizeof($spesplit); $i++){
    $querysplit = $spesplit[$i];
    $sql = "SELECT * FROM graduated WHERE  college like '%$querysplit%' and eventnotification ='Yes'  order by id DESC limit 3";
    ...
    if ($result-> num_rows > 0 ){
      while ($row=mysqli_fetch_array($result)){
        ...
        if(send_mail($recipient,$subject, $message)){
                    ...
                    $_SESSION['guidanceeventsuccess'] = "asdasdsa";
