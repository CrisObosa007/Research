...
// unset($_SESSION['send']);
    ...
    if (isset($_SESSION['send'])) {
        ...
        $yeargraduated = $_SESSION['yeargraduated'];
  ...
  <div class="limiter" id="form2" style="display: block;">
    <div class="container-login100" style="background-color: #07435f;">
        <div class="wrap-login200 p-l-50 p-r-50 p-t-50 p-b-50" style="background-color:#F7F5F5">
         ...
         <form action='controllerUserEmail.php' class='form' method="POST" name="contactForm" onsubmit="return validateForm()">
            <?php 
            $existingemail = "";
...
<p class='field  half' style="position:relative;;">
    ...
    <input type="number" class='text-input' id='idno' name='idno'  type='number' placeholder="ID Number"  style="color: #fff;background-color: #0c506f;padding-left: 35px;" value="<?php echo $idno ?>">
    ...
    <select class="text-input" name="course"  style="color: #fff;background-color: #0c506f;padding-left: 35px;">
        ...
        while ($row=mysqli_fetch_array($result)){
           ...
           if ($row['course'] == $course) {
            echo $selectcourse = "selected";
        }
        echo "<option  value='".$row['course']."'".$selectcourse.">".$row['course']."</option>";
...
<p class='field half' style="height:97px;background-color: transparent;position: relative;" >
    ...
    <select  id='specialization' name="specialization[]" class="specialization" value="" multiple>
        ...
        if ($result-> num_rows > 0 ){
          while ($row=mysqli_fetch_array($result)){
            $skillsplit = explode(', ', $specialization);  
...
<p class='field half' style="background-color:transparent;position: relative;">
    ...
    <select class="text-input" name="classification" id="classification"  style="color: #fff;background-color: #0c506f;padding-left: 35px;">
        <option disabled selected value="">Select</option>
        <option value="Graduated" <?php if ($classification == "Graduated") {echo "selected";} ?>>Alumni</option>
        <option value="Graduating" <?php if ($classification == "Graduating") {echo "selected";} ?>>Graduating</option>
    </select>
...
<p class='field half'  style="position:relative;">
    ...
    <select class="text-input" name="yeargraduated"  style="color: #fff;background-color: #0c506f;padding-left: 35px;">
        ...
        for ($i = $date; $i > 1980; $i--)  {
            ...
            if ($yeargraduated == $ii) {
               $selectedgraduated1 = "selected";
           }
           echo "<option value='".$ii."' style='display:none' class='graduated1' ".$selectedgraduated1.">".$ii."</option>";
       }
       ...
       for ($i = $date; $i < $date2; $i++)  {
        $ii = $i - 1;
        if ($yeargraduated == $ii) {
           $selectedgraduated2 = "selected";
       }

       echo "<option value='".$ii."' style='display:none' class='graduated2' ".$selectedgraduated2.">".$ii."</option>";
   }
...
<p class='field  half' style="position:relative;">
    ...
    <input class='text-input' id='lastname' name='lastname' placeholder="Last Name" style="color: #fff;background-color: #0c506f;padding-left: 35px;"value="<?php echo $lastname ?>">
...
<p class='field half' style="position:relative;">
    ...
    <input class='text-input' id='firstname' name='firstname' placeholder="First Name"  style="color: #fff;background-color: #0c506f;padding-left: 35px;" value="<?php echo $firstname; ?>">
    <label id="firstnameErr" class="failed"></label>
    <i class="fa fa-user iconlabel"></i>
</p>
<p class='field half 'style="position:relative;">
    <label class='label'>Middle Name</label>
    <input class='text-input' id='middlename' name='middlename' placeholder="Middle Name"  style="color: #fff;background-color: #0c506f;padding-left: 35px;"value="<?php echo $middlename ?>">
...
<p class='field  half' style="position:relative;">
    ...
    <input type="radio" name="gender" value="Male" <?php if ($gender == "Male") {echo "checked";} ?>  style="height: 15px;width: 15px;margin-right: 5px;">
    <label style="font-size: 17px;color: #0c506f;margin-right: 20px;">Male</label>
    <input type="radio" name="gender" value="Female"<?php if ($gender == "Female") {echo "checked";} ?> style="height: 15px;width: 15px;margin-right: 5px;">
...
<p class='field  secondhalf' style="position:relative;">
    ...
    <input type="text" class='text-input' id='email' name='email' placeholder="eg.ucu@gmail.com"  style="color: #fff;background-color: #0c506f;padding-left: 35px;" value="<?php echo $email ?>">
    <label id="emailErr" class="failed"></label>
    <i class="fa fa-envelope iconlabel"></i>
</p>
<p class='field  half' style="position:relative;">
    <label class='label' >Contact # <label class="requiredlabel">*</label></label>
    <input type="number" class='text-input' id='contact' name='contact'  placeholder="eg. 09919290121"  style="color: #fff;background-color: #0c506f;padding-left: 35px;"  value="<?php echo $contact ?>">
...
<p class='field half' style="position:relative;">
    ...
    <input class='text-input' id='date' name='birthdate' type='date'  style="color: #fff;background-color: #0c506f;color-scheme: dark;"  value="<?php echo $birthdate ?>">
...
<p class='field half' style="position:relative;">
    ...
    <select id="region" class="text-input" name="region1"  style="color: #fff;background-color: #0c506f;padding-left: 35px;"  onchange="CheckRegion()">
        <option value="" selected disabled>Select</option>
        <?php 

        $sql = "select * from refregion order by regDesc ASC";
        $result = $conn-> query($sql);

        while ($row=mysqli_fetch_array($result)){
         if (isset($_SESSION['send'])) {
            $selectregion = "";
            if ($row['regCode'] == $region1) {
                echo $selectregion = "selected";
            }
            echo "<option value='".$row['regCode']."' ".$selectregion.">".$row['regDesc']."</option>";
...
<p class='field half' style="position:relative;display: block;" id="aaaaa">
    <label class='label'>Province <label class="requiredlabel">*</label></label>
    <input type="hidden" name="province"/>
    <select id="province" class="text-input" name="province1"  style="color: #fff;background-color: #0c506f;padding-left: 35px;"   onchange="CheckProvince()">
        <option value="" selected disabled>Select</option>
        <?php 
        ...
        while ($row=mysqli_fetch_array($result)){
         if (isset($_SESSION['send'])) {
            ...
            if ($row['regCode'] == $region1) {
                $selectprovinceblock = "display:block";
            }
            if ($row['provCode'] == $province1) {
             $selectprovince = "selected";
         }

         echo "<option value='".$row['provCode']."' style=';display:none;".$selectprovinceblock."' class='provinceall".$row['regCode']."'".$selectprovince.">".$row['provDesc']."</option>";
     }else{
        echo "<option value='".$row['provCode']."' style='display:none' class=' provinceall ".$row['regCode']."'  >".$row['provDesc']."</option>";
    }
}
...
<p class='field half' style="position:relative;">
    ...
    <select id="city" class="text-input" name="city1"  style="color: #fff;background-color: #0c506f;padding-left: 35px;" onchange="CheckCity()">
     <option value="" selected disabled>Select</option>                
     ...
     while ($row=mysqli_fetch_array($result)){
         if (isset($_SESSION['send'])) {
            ...
            $selectcity = "";
            if ($row['provCode'] == $province1) {
                $selectcityblock = "display:block";
            }
            if ($row['citymunCode'] == $city1) {
             $selectcity = "selected";
         }

         echo "<option value='".$row['citymunCode']."' style='display:none".$selectcityblock."' class=' cityall ".$row['provCode']."'".$selectcity.">".$row['citymunDesc']."</option>";
     }else{
       echo "<option value='".$row['citymunCode']."' style='display:none' class=' cityall ".$row['provCode']."'  >".$row['citymunDesc']."</option>";
   }
}
...
<p class='field half' style="position:relative;">
    ...
    <select id="barangay" class="text-input" name="barangay1"  style="color: #fff;background-color: #0c506f;padding-left: 35px;"> 
        <option value="" selected disabled>Select</option>
        <?php 
        $sql = "select * from refbrgy order by brgyDesc ASC";
        $result = $conn-> query($sql);
        ...
        while ($row=mysqli_fetch_array($result)){
         if (isset($_SESSION['send'])) {
           ...
           $selectbrgy = "";
           if ($row['citymunCode'] == $city1) {
            $selectbrgyblock = "display:block";
        }
        if ($row['brgyCode'] == $barangay1) {
         $selectbrgy = "selected";
     }
     echo "<option value='".$row['brgyCode']."' style='display:none;".$selectbrgyblock."' class=' brgyall ".$row['citymunCode']."'".$selectbrgy.">".$row['brgyDesc']."</option>";
 }else{
    echo "<option value='".$row['brgyCode']."' style='display:block' class=' brgyall ".$row['citymunCode']."'  >".$row['brgyDesc']."</option>";
}
}
...
<p class='field  secondhalf' style="position:relative">
    ...
    <input type="text" class='text-input' id='house' name='house'  type='number' placeholder="Street/House #"  style="color: #fff;background-color: #0c506f;padding-left: 35px;"value="<?php echo $house ?>">
...
<script type="text/javascript">
    ...
    function CheckRegion(){
     ...
     var x = document.getElementsByClassName("provinceall");
     var i;
     for (i = 0; i < x.length; i++) {
        x[i].style.display = 'none';
    }

    var x = document.getElementsByClassName(varRegion);
    var i;
    for (i = 0; i < x.length; i++) {
...
function CheckProvince(){
 ...
 var varProvince =  document.getElementById('province').value;

 var x = document.getElementsByClassName("cityall");
 var i;
 for (i = 0; i < x.length; i++) {
    x[i].style.display = 'none';
}

var x = document.getElementsByClassName(varProvince);
var i;
for (i = 0; i < x.length; i++) {
    x[i].style.display = 'block';
}
...
function CheckCity(){
 ...
 var x = document.getElementsByClassName("brgyall");
 var i;
 for (i = 0; i < x.length; i++) {
    x[i].style.display = 'none';
}

var x = document.getElementsByClassName(varCity);
var i;
for (i = 0; i < x.length; i++) {
    x[i].style.display = 'block';
}
    ...
    <div class="" style="width: auto;position: relative;"><label class="labelterm1"></label>
        <div id="checkboxpopup" style="position: relative;width: auto;display: inline;">
           ...
           <div style="width:100%; display: flex;">
            ...
            <img src="images/agree.png" style="width:15px;height: 15px;margin-right: 5px;display: none;             <?php 
            if (isset($_SESSION['send'])) {
                echo "display:block";
            }
        ?>" id="imageagree">
